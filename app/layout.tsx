...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Sidebarn fokuserar nu på inställningar och tysta sidor
  const sidebarItems = [
    ...
    <html lang="sv">
      <body style={{ 
        ...
        <div className={`app-container ${isSidebarOpen ? 'pushed' : ''}`}>
          ...
          {/* DYNAMISK BOTTOM NAV - LÅST NAVIGATION */}
          <nav className="mobile-nav">
             ...
             <Link href="/buy" className={`tab ${pathname.includes('/buy') ? 'active' : ''}`}><span>🛍️</span><small>Utforska</small></Link>
             <Link href="/chat" className={`tab ${pathname.includes('/chat') ? 'active' : ''}`}><span>💬</span><small>Chatt</small></Link>
          </nav>
        </div>

        <style jsx global>{`
          /* Din befintliga styling men med fixade transitions */
          .sidebar-drawer { position: fixed; left: -280px; top: 0; width: 280px; height: 100%; background: #05070a; z-index: 2000; transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1); border-right: 1px solid #111; padding-top: env(safe-area-inset-top); }
          ...
          .mobile-nav { position: fixed; bottom: 0; width: 100%; height: calc(80px + env(safe-area-inset-bottom)); background: rgba(2,4,10,0.98); backdrop-filter: blur(20px); display: flex; border-top: 1px solid rgba(255,255,255,0.05); z-index: 1000; padding-bottom: env(safe-area-inset-bottom); }
          ...
          .center-fab button { position: absolute; top: -25px; left: 50%; transform: translateX(-50%); width: 60px; height: 60px; background: var(--neon-purple); border-radius: 22px; border: 5px solid #02040a; color: #fff; font-size: 30px; }
        `}</style>
      </body>
    </html>
  );
