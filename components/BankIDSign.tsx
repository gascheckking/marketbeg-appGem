...
export default function BankIDSign({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <div style={{ padding: "40px 30px", textAlign: "center", background: "#fff", borderRadius: "32px", color: "#000", maxWidth: "340px", margin: "20px auto" }}>
      ...
      {status === "waiting" ? (
        ...
        <div style={{ animation: "popIn 0.4s ease-out" }}>
          ...
          <p style={{ fontSize: "11px", color: "#1DB954", fontWeight: 900, marginTop: "5px" }}>KARMA DNA UPPDATERAT</p>
        </div>
      )}

      <style jsx>{`
        .loader {
          width: 35px; height: 35px; border: 4px solid #f0f0f0; border-top: 4px solid #111;
